...
def calc_sky_annulus(data, x, y, r_in, r_out, sky_method='median', 
    ...
    if sky_method in ['mean', 'median', 'mode']:
        ...
        try:
            ...
            try: 
                A, mu, sigma = calc_1d_gauss_background(data, bins=100, hist_range=(-20, 10))
            except RuntimeError:
                try: 
                    A, mu, sigma = calc_1d_gauss_background(data, bins=100, hist_range=(-10, 20))
                except RuntimeError:
                    try: 
                        A, mu, sigma = calc_1d_gauss_background(data, bins=100, hist_range=(-5, 5))
                    except RuntimeError:
