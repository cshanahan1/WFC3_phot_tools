...
def make_cr_mask(data, nlen = 4, ncomp = 10, mult = 4.):
	...
	sigma above the median of ncomp pixels +nlen ablve and -nlen below them.
	It is expected that valid pixels may be above the median of pixels at
...
def mask_and_repair_flagged_pixels(data, scan_orient):
	"""Given a 2D array of data, as well as the orientation of the scan
		...
		the 2 nearest surrounding 'good' pixels above or below, in the direction
		of the scan.
	...
	if len(k_min[0]) > 1:
		k_min = [np.array(item[0]) for item in k_min]  #select only one of these, sometimes it will find two nearby

	gr_max = np.nanmax(gr.flatten())
	k_max = np.where(gr == gr_max) #y,x where max occurs
	if len(k_max[0]) > 1:
		k_max = [np.array(item[0]) for item in k_max]  #select only one of these, sometimes it will find two nearby

	mult = 5
	dy = 5
	print('KMIN, KMAX', k_min, k_max)
	for j in range(min([k_min[1],k_max[1]])[0]-3,max([k_min[1],k_max[1]])[0]+3+1):
		v = data[:,j]
		mv = medfilt(v,5)
		v_mv = v - mv
		sig = np.nanstd(v_mv)
		...
		if len(k) > 0:
			for i, val in enumerate(k):
				if ((np.abs(k[i]-k_min[0]) < dy) or (np.abs(k[i]-k_max[0]) < dy)):
					mask[k,j] = 1
...
def make_crcorr_file_scan_wfc3(input_file, output_dir = '', ext = 1,
						   	   write_mask = True):
	...
	output_dir : str
		Directory where corrected files should be output. Defaults to input_file
		directory.
	ext : int
	...
	else: #add trailing slash to directory if it doesn't exist
		if output_dir[-1] != '/':
